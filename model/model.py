...
def get_decoder():
    ...
    decoder.load_state_dict(torch.load(WEIGHTS_FOLDER / 'decoder_iter_500000.pth'))
    return decoder
...
def get_encoder():
    ...
    encoder.load_state_dict(torch.load(WEIGHTS_FOLDER / 'vgg_normalised.pth'))
    return encoder

def get_transformer():
    tfm =Transform(in_planes=512)
    tfm.load_state_dict(torch.load(WEIGHTS_FOLDER / 'transformer_iter_500000.pth'))
    return tfm
...
class Encoder(nn.Module):
    def __init__(self,):
        super().__init__()
        full_encoder = get_encoder()
        self.enc_4 = nn.Sequential(
            *list(full_encoder.children())[:31]
        )
...
class Stylizer(nn.Module):
    ...
    def pil_to_arr(self,pil):
        return transforms.ToTensor()(pil).unsqueeze(0).to(self.device)

    def arr_to_pil(self,arr):
        res = arr.detach().clone().cpu()[0].permute(1,2,0).clamp(0,1).numpy()
        res = Image.fromarray((res * 255).astype('uint8'))
        return res
        
    @torch.no_grad()
    def forward(self, content, style,  iters = 1):
        style = self.pil_to_arr(style)
        content = self.pil_to_arr(content)
        ...
        for _ in range(iters):
            ...
            c4_1, c5_1 = self.encoder(c4_1)

            s4_1, s5_1 = self.encoder(s4_1)
        ...
        return self.arr_to_pil(c_stylized)
